...
const emptyCoupon = (): Partial<DBCoupon> => ({
  code: "", discount: 10, maxUses: 100, isActive: true, expiresAt: null, description: "",
});
...
export function StoreAdminClient({ user }: Props) {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      else if (r.status === 401) showMsg("err", "انتهت الجلسة، أعد تسجيل الدخول");
    } catch { showMsg("err", "فشل تحميل المنتجات"); }
  }, []);
  ...
  const saveMaintenance = async () => {
    ...
    try {
      ...
      showMsg("ok", maintenance ? "وضع الصيانة مفعّل ✓" : "الموقع يعمل بشكل طبيعي ✓");
    } catch (e) { showMsg("err", (e as Error).message || "فشل الحفظ"); }
  ...
  const saveProduct = async () => {
    if (!editProduct.nameAr?.trim()) { showMsg("err", "اسم المنتج بالعربي مطلوب"); return; }
    if (!editProduct.name?.trim()) { showMsg("err", "اسم المنتج بالإنجليزي مطلوب"); return; }
    setSaving(true);
    try {
  ...
  const saveCoupon = async () => {
    if (!editCoupon.code?.trim()) { showMsg("err", "كود الخصم مطلوب"); return; }
    setSaving(true);
    try {
      const url = isEditingCoupon ? `/api/admin/coupons/${editCoupon.id}` : "/api/admin/coupons";
      const r = await fetch(url, {
        method: isEditingCoupon ? "PATCH" : "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen bg-[#09090C]" dir="rtl">
      ...
      {/* Toast notification */}
      {msg && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-2 px-5 py-3 rounded-xl shadow-2xl text-sm font-semibold animate-fade-in ${msg.type === "ok" ? "bg-green-700 text-white" : "bg-red-700 text-white"}`}>
      ...
      {/* Header */}
      <header className="bg-[#0D0D10] border-b border-[#C9A84C]/10 px-5 py-3 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[#5A5045] hover:text-red-400 transition-colors text-xs px-3 py-1.5 rounded-lg hover:bg-red-500/10"
          >
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${
                tab === t.id
                  ? "bg-[#C9A84C]/15 border border-[#C9A84C]/30 text-[#E8C96A]"
                  : "bg-[#1A1A1F] border border-[#2A2A32] text-[#6B6558] hover:text-[#C9A84C] hover:border-[#C9A84C]/20"
              }`}
            >
              <t.icon size={16} />
              <span>{t.label}</span>
          ...
          <button
            ...
            title="تحديث"
            className="mr-auto p-2.5 rounded-xl bg-[#1A1A1F] border border-[#2A2A32] text-[#5A5045] hover:text-[#C9A84C] hover:border-[#C9A84C]/20 transition-all"
          >
            <RefreshCw size={16} />
          </button>
        </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {products.map((p) => (
                  <GlassCard key={p.id} className="p-4 relative overflow-hidden">
                    {!p.isAvailable && <div className="absolute top-0 left-0 right-0 h-0.5 bg-red-500/60" />}
                    {p.isFeatured && <div className="absolute top-0 left-0 right-0 h-0.5 bg-[#C9A84C]" />}

                    <div className="flex items-start justify-between mb-3">
                      <div className="min-w-0 flex-1">
                        <p className="text-[#F5F0E8] font-bold text-sm truncate">{p.nameAr}</p>
                        <p className="text-[#5A5045] text-xs truncate">{p.name}</p>
                      </div>
                    ...
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      {p.isFeatured && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-[#C9A84C]/15 text-[#E8C96A] flex items-center gap-0.5">
                          <Star size={9} />مميز
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2 pt-3 border-t border-[#2A2A32]">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 py-2 rounded-lg text-xs font-semibold bg-[#1A1A1F] border border-[#2A2A32] hover:border-[#C9A84C]/30 text-[#8A8070] hover:text-[#C9A84C] transition-all"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 py-2 rounded-lg text-xs font-semibold bg-[#C9A84C]/10 border border-[#C9A84C]/20 text-[#C9A84C] hover:bg-[#C9A84C]/20 transition-all"
                      >
                ...
                coupons.map((c) => (
                  <GlassCard key={c.id} className="p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-xs text-[#5A5045] flex-wrap">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        ...
                        className={`p-2 rounded-lg border transition-all ${c.isActive ? "bg-green-500/10 border-green-500/20 text-green-400 hover:bg-green-500/20" : "bg-[#1A1A1F] border-[#2A2A32] text-[#5A5045] hover:text-green-400"}`}
                      >
                        {c.isActive ? <ToggleRight size={16} /> : <ToggleLeft size={16} />}
                      </button>
                      <button
                        onClick={() => openEditCoupon(c)}
                        className="p-2 rounded-lg bg-[#C9A84C]/10 border border-[#C9A84C]/20 text-[#C9A84C] hover:bg-[#C9A84C]/20 transition-all"
                      >
                        <Pencil size={16} />
                      </button>
                      <button
          ...
          <GlassCard className="p-6 max-w-lg">
            ...
            {maintenance && (
              <div className="flex items-center gap-2 p-3 rounded-xl bg-red-500/10 border border-red-500/20 mb-5">
                ...
                <p className="text-red-400 text-sm font-semibold">الموقع تحت الصيانة حالياً</p>
              </div>
            )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 rounded-xl bg-[#1A1A1F] border border-[#2A2A32]">
                ...
                >
                  <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all duration-200 ${maintenance ? "right-0.5" : "left-0.5"}`} />
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <GlassCard className="w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 relative">
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setProductModal(false)} className="text-[#5A5045] hover:text-[#C9A84C] p-1 rounded-lg hover:bg-[#C9A84C]/10 transition-all">
                <X size={20} />
              </button>
            </div>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={editProduct.nameAr ?? ""} onChange={(e) => setEditProduct(p => ({ ...p, nameAr: e.target.value }))} className={inputCls} placeholder="حزمة الذهب" />
                </div>
                <div>
                  <label className={labelCls}>الاسم بالإنجليزي *</label>
                  <input value={editProduct.name ?? ""} onChange={(e) => setEditProduct(p => ({ ...p, name: e.target.value }))} className={inputCls} placeholder="Gold Bundle" dir="ltr" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>السعر (ريال) *</label>
                  <input type="number" value={editProduct.price ?? 0} onChange={(e) => setEditProduct(p => ({ ...p, price: Number(e.target.value) }))} className={inputCls} min={0} />
                </div>
                <div>
                  <label className={labelCls}>الخصم % (اختياري)</label>
                  <input type="number" value={editProduct.discount ?? ""} onChange={(e) => setEditProduct(p => ({ ...p, discount: e.target.value ? Number(e.target.value) : undefined }))} className={inputCls} min={0} max={100} placeholder="10" />
                </div>
              </div>
              <div>
                <label className={labelCls}>الوصف بالعربي</label>
                <textarea value={editProduct.descriptionAr ?? ""} onChange={(e) => setEditProduct(p => ({ ...p, descriptionAr: e.target.value }))} className={`${inputCls} resize-none`} rows={2} />
              </div>
              <div>
                <label className={labelCls}>مسار الصورة</label>
                <input value={editProduct.image ?? "/HQN.png"} onChange={(e) => setEditProduct(p => ({ ...p, image: e.target.value }))} className={inputCls} dir="ltr" placeholder="/products/bundle.png" />
              </div>
              <div>
                <label className={labelCls}>المميزات (كل سطر ميزة)</label>
                <textarea
              ...
              <div className="flex items-center gap-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={editProduct.isAvailable ?? true} onChange={(e) => setEditProduct(p => ({ ...p, isAvailable: e.target.checked }))} className="accent-[#C9A84C] w-4 h-4" />
                  <span className="text-[#B0A890] text-sm">متاح للبيع</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={editProduct.isFeatured ?? false} onChange={(e) => setEditProduct(p => ({ ...p, isFeatured: e.target.checked }))} className="accent-[#C9A84C] w-4 h-4" />
                  <span className="text-[#B0A890] text-sm flex items-center gap-1"><Star size={12} className="text-[#C9A84C]" />مميز</span>
                </label>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <Button onClick={saveProduct} loading={saving} className="flex-1 gap-1">
                <Save size={16} /><span>{isEditingProduct ? "حفظ التعديلات" : "إضافة المنتج"}</span>
              </Button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <GlassCard className="w-full max-w-md p-6 relative">
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setCouponModal(false)} className="text-[#5A5045] hover:text-[#C9A84C] p-1 rounded-lg hover:bg-[#C9A84C]/10 transition-all">
                <X size={20} />
              </button>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditCoupon(c => ({ ...c, code: e.target.value.toUpperCase().replace(/\s+/g, "") }))}
                  className={`${inputCls} font-mono uppercase tracking-widest`}
                  placeholder="HQNVIP20"
                  dir="ltr"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={editCoupon.discount ?? 10} onChange={(e) => setEditCoupon(c => ({ ...c, discount: Number(e.target.value) }))} className={inputCls} min={1} max={100} />
                </div>
                <div>
                  <label className={labelCls}>أقصى استخدام</label>
                  <input type="number" value={editCoupon.maxUses ?? 100} onChange={(e) => setEditCoupon(c => ({ ...c, maxUses: Number(e.target.value) }))} className={inputCls} min={1} />
                </div>
              </div>
              <div>
                ...
                <input
                  type="datetime-local"
                  value={editCoupon.expiresAt ? editCoupon.expiresAt.slice(0, 16) : ""}
                  onChange={(e) => setEditCoupon(c => ({ ...c, expiresAt: e.target.value || null }))}
                  className={inputCls}
                  dir="ltr"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className={inputCls}
                  placeholder="كوبون خاص بأعضاء VIP"
                />
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={editCoupon.isActive ?? true} onChange={(e) => setEditCoupon(c => ({ ...c, isActive: e.target.checked }))} className="accent-[#C9A84C] w-4 h-4" />
                <span className="text-[#B0A890] text-sm">مفعّل</span>
              </label>
            </div>
            <div className="flex gap-3 mt-6">
              <Button onClick={saveCoupon} loading={saving} className="flex-1 gap-1">
                <Save size={16} /><span>{isEditingCoupon ? "حفظ التعديلات" : "إنشاء الكوبون"}</span>
              </Button>
